...
부동산 매칭 플랫폼의 주요 기능을 담당하는 REST API를 설계하고 구현했습니다.

주요 개발 기능

- 매물 요청 수정 / 조회 API
- 매물 내놓기 등록 / 수정 / 조회 / 삭제 API
- 알림 기능 (서비스 앱푸시,알림톡, 방해금지 시간 설정)
- 관리자 매물 / 회원 관리 API / 메모,블랙리스트 등 API
...
관리자 매물 조회 API에서 복잡한 조건 필터링을 위해 다중 서브쿼리가 사용되면서 조회 성능 저하 문제가 발생했습니다.
...
이로 인해 실제 사용 여부와 관계없이 번호 유지 비용이 발생하는 문제가 있었습니다.
...
- 해지된 번호 **다른 중개사에게 재사용 가능하도록 구조 설계**
